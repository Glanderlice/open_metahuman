...
def start_engine(model_root: Union[str, Path], facedb_path: Union[str, Path] = None):
    ...
    return {'processor': processor, 'recognizer': recognizer}


def scan_faces(video_file: Union[str, Path], engine: Dict[str, Any], sampling_fps: Union[int, float] = 2, k: int = 10):
...
def image2face(image_file: Union[str, Path], engine: Dict[str, Any]):
...
def postprocess_faces(video_file: Union[str, Path], engine: Dict[str, Any], swap_faces: Dict[str, Face],
                      enhance: bool = True,
...
def rename_faces(engine: Dict[str, Any], names: List[Tuple[str, str]]):
...
def save_facedb(engine: Dict[str, Any], db_path: Union[str, Path]):
    """保存人脸库"""
    facedb: SimpleFaceDB = engine['recognizer'].facedb
...
def run():
    ...
    # 遍历视频记录人脸
    faces: List[Tuple[str, str]] = scan_faces(data_root / 'video/movie.mp4', engine=engine, sampling_fps=2, k=10)
